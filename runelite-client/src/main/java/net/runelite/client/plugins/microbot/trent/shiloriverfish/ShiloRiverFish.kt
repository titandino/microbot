...
private class Root : State() {
    ...
    override fun loop(client: Client, script: StateMachineScript) {
        if (Rs2Inventory.isFull()) {
            val banker = Rs2NpcQueryable().withName("banker").nearest()
        ...
        val fishingSpot = Rs2NpcQueryable().withName("rod fishing spot").nearest()
        if (fishingSpot == null && Rs2Walker.walkTo(WorldPoint(2846, 2970, 0), 10)) {
            sleep(1260, 5920)
            return
        }
        if (fishingSpot != null && fishingSpot.click("lure")) {
